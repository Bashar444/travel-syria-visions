          ...
          <div className={`hidden md:flex items-center space-x-1 rtl:space-x-reverse ${direction === 'rtl' ? 'order-1' : 'order-2'}`}>
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className={`relative px-4 py-3 rounded-md text-sm font-medium transition-all duration-300 hover:bg-gray-50 hover:shadow-sm hover:-translate-y-0.5 ${
                  isActivePage(item.href)
                    ? 'text-[#EE3524] after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-[#EE3524] after:content-[""]'
                    : 'text-[#231F20] hover:text-[#EE3524]'
                }`}
              >
                {item.name}
              </Link>
            ...
            <Button
              ...
              className="border-[#EE3524] text-[#EE3524] hover:bg-[#EE3524] hover:text-white transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 mx-2"
            >
              {language === 'ar' ? 'EN' : 'عربي'}
            </Button>
            ...
            {/* WhatsApp CTA */}
            <Button 
              className="bg-[#EE3524] hover:bg-red-600 text-white transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 transform"
                ...
                <div className="flex flex-col space-y-1 mt-8">
                  ...
                  {navigation.map((item) => (
                    <Link
                      ...
                      className={`px-4 py-3 rounded-md text-base font-medium transition-all duration-200 ${
                        isActivePage(item.href)
                          ? 'text-[#EE3524] bg-red-50 border-r-2 border-[#EE3524]'
                  ...
                  <div className="pt-4 border-t border-gray-200 mt-4">
                    <Button
                      variant="outline"
                      onClick={handleLanguageSwitch}
                      className="border-[#EE3524] text-[#EE3524] hover:bg-[#EE3524] hover:text-white w-full mb-3 transition-all duration-200"
                    >
                      {language === 'ar' ? 'English' : 'عربي'}
                    </Button>

                    <Button 
                      className="bg-[#EE3524] hover:bg-red-600 text-white w-full transition-all duration-200"
                      onClick={() => {
